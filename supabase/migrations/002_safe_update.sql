-- ============================================================
-- Safe update — drops and recreates policies, skips existing tables
-- ============================================================
...
CREATE POLICY "Authenticated users can view all memories" ON memories FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Users can insert own memories" ON memories FOR INSERT WITH CHECK (auth.uid() = created_by);
CREATE POLICY "Users can update own memories" ON memories FOR UPDATE USING (auth.uid() = created_by);
CREATE POLICY "Users can delete own memories" ON memories FOR DELETE USING (auth.uid() = created_by);
...
CREATE POLICY "Authenticated users can view all events" ON events FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Users can insert own events" ON events FOR INSERT WITH CHECK (auth.uid() = created_by);
...
CREATE POLICY "Users can view notes they sent or received" ON notes FOR SELECT USING (auth.uid() = sender_id OR auth.uid() = receiver_id);
CREATE POLICY "Users can insert notes as sender" ON notes FOR INSERT WITH CHECK (auth.uid() = sender_id);
CREATE POLICY "Users can update their own notes" ON notes FOR UPDATE USING (auth.uid() = sender_id OR auth.uid() = receiver_id);
CREATE POLICY "Users can delete their own sent notes" ON notes FOR DELETE USING (auth.uid() = sender_id);
...
CREATE POLICY "Authenticated users can view all media" ON media FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Users can insert own media" ON media FOR INSERT WITH CHECK (auth.uid() = uploaded_by);
CREATE POLICY "Users can delete own media" ON media FOR DELETE USING (auth.uid() = uploaded_by);
...
CREATE POLICY "Authenticated users can view all reactions" ON memory_reactions FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Users can manage their own reactions" ON memory_reactions FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE TRIGGER set_memories_updated_at BEFORE UPDATE ON memories FOR EACH ROW EXECUTE FUNCTION set_updated_at();
